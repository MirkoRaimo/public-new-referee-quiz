...
final questionsListReducer = combineReducers<QuestionsListState>([
  TypedReducer<QuestionsListState, AnswerQuestion>(_answeringQuestion)
      as QuestionsListState Function(QuestionsListState, dynamic),
  TypedReducer<QuestionsListState, SetQuestionsListContext>(_settingContext)
      as QuestionsListState Function(QuestionsListState, dynamic),
  TypedReducer<QuestionsListState, GenerateTrueFalseQuestions>(
          _generateTrueFalseQuestions)
      as QuestionsListState Function(QuestionsListState, dynamic),
  TypedReducer<QuestionsListState, GenerateQuizQuestions>(
          _generateQuizQuestions)
      as QuestionsListState Function(QuestionsListState, dynamic),
]);

QuestionsListState _answeringQuestion(
...
QuestionsListState _settingContext(
    QuestionsListState state, SetQuestionsListContext action) {
  return state.copyWith(context: action.context);
}

QuestionsListState _generateTrueFalseQuestions(
    QuestionsListState state, GenerateTrueFalseQuestions action) {
  const String _theAnswerIs = "The answer is:";

  List<Question> listOfQuestions = []
    ..add(Question.trueFalse(
        questionStatement: "$_theAnswerIs $STR_TRUE",
        correctAnswer: Question.TRUE))
    ..add(Question.trueFalse(
        questionStatement: "$_theAnswerIs $STR_FALSE",
        correctAnswer: Question.FALSE))
    ..add(Question.trueFalse(
        questionStatement: "$_theAnswerIs $STR_TRUE",
        correctAnswer: Question.TRUE))
    ..add(Question.trueFalse(
  ...
  return state.copyWith(questionsList: listOfQuestions);
}

QuestionsListState _generateQuizQuestions(
    QuestionsListState state, GenerateQuizQuestions action) {
  const String _theAnswerIs = "The answer is:";

  List<Question> listOfQuestions = []
    ..add(Question(
        questionStatement: "$_theAnswerIs $STR_TRUE",
        possibleAnswers: [STR_TRUE, STR_FALSE, STR_IMPOSSIBLE],
        correctAnswer: Question.TRUE))
    ..add(Question(
        questionStatement: "$_theAnswerIs $STR_FALSE",
        possibleAnswers: [STR_TRUE, STR_FALSE, STR_IMPOSSIBLE],
