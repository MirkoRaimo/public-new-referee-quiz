...
Widget recapAnswersList (BuildContext context, List<Question> questions){
  return ListView.builder(
      shrinkWrap: true,
      itemCount: (questions!= null && questions.isNotEmpty) ? questions.length : 0,
...
Widget recapAnswers (BuildContext context, QuestionsListState questionsListState, {String msgToShow}){
  ...
  return Column(
    ...
    children: [
      Text("$msgToShow ${questionsListState.indexesCorrectQuestions.length} / ${questionsListState.questionsList.length}"),
      Flexible(flex: 1, child: recapAnswersList(context, questionsListState.questionsList))
    ],
  );
}
